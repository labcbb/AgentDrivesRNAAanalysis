...
def _run_bowtie_one(
    ...
    bam_path = out_dir / f"{sample}.bam"
    sam_path = bam_path.with_suffix(".sam")

    # Skip if output already exists
    if bam_path.exists() and bam_path.stat().st_size > 0:
        print(f"[bowtie] Skipping {sample}: {bam_path.name} already exists", flush=True)
        log_out = str(out_dir / f"{sample}.bowtie.log")
        return {"sample": sample, "sam": str(sam_path), "bam": str(bam_path), "log": log_out, "metrics": {}}
    ...
    if not sam_path.exists():
        raise RuntimeError(f"bowtie failed to produce {sam_path.name}")
...
def bowtie(
    ...
    for result in results:
        ...
        adata.obs.loc[sample_name, "bam_path"] = result["bam"]
