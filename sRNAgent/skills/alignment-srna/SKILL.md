...
> ⚠️ **output_dir 约定**：批量比对一律输出到 **`aligned/`**（本 skill 所有示例统一）。"严谨比对"（2a）和"容错比对"（2b）是**两种备选参数**，**不是两条独立流程**——默认只选其一跑一次，**不要**创建 `aligned_strict`、`aligned_perm` 之类的变体目录。除非用户明确要求同时对比两种模式，否则一次比对、一个目录。
...
adata.obs["sam_path"]                    # 临时 SAM 文件路径（自动转换为 BAM 后删除）
adata.obs["bam_path"]                    # BAM 文件路径（最终结果，sorted + indexed）
...
print(adata.obs[["bam_path", "bowtie_alignment_rate"]])
```
