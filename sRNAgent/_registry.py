"""
Function Registry System for sRNAgent.

Decorator-based registration so Agent tools can discover sRNA-seq APIs via
natural language queries, aliases, and keyword search.
"""
...
class FunctionRegistry:
    """Central registry for all @register_function-decorated sRNAgent APIs."""
    ...
    def _ensure_hydrated(self) -> None:
        """Import sRNAgent submodules once so queries see all registered functions.

        Registration happens via ``@register_function`` import side effects.
        Hydration scans the package tree relative to this file on first query.
        """
        if self._hydrated:
            return
        self._hydrated = True
...
def _package_root() -> Path:
    """Return the sRNAgent package directory (relative to this file)."""
    return Path(__file__).resolve().parent


def _discover_hydration_modules() -> List[str]:
    """Discover importable public modules under sRNAgent using relative paths."""
    import sys
    ...
    for py_file in sorted(root.rglob("*.py")):
        ...
        if py_file.name == "__init__.py":
            continue
        rel_parts = py_file.relative_to(project_root).with_suffix("").parts
        modules.append(".".join(rel_parts))
    return modules


def _hydrate_registry_for_export() -> None:
    """Import all public sRNAgent modules so @register_function side effects run."""
    import importlib

    for module_name in _discover_hydration_modules():
        try:
...
def get_function_help(query: str) -> None:
    ...
    --------
    >>> import sRNAgent as sa
    >>> sa.get_function_help("fastq_dl")
    >>> sa.get_function_help("SRA下载")
    """
    results = _global_registry.find(query)
...
def recommend_function(task_description: str, n: int = 5) -> List[Callable]:
    ...
    --------
    >>> import sRNAgent as sa
    >>> sa.recommend_function("download sRNA-seq FASTQ from SRA")
    >>> sa.recommend_function("ENA 下载")
    """
...
def export_registry(filepath: Optional[str] = None, 
    ...
    --------
    >>> # Export to file
    >>> sa.export_registry('registry.json')
    ...
    export_metadata = {
        ...
        'srnagent_version': getattr(__import__('sRNAgent'), '__version__', 'unknown')
    }
