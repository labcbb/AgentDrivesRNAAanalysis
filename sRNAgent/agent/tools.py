...
def manage_visualization(
    ...
    *,
    ...
    """Show/hide plots on the UI Visualization board for the active chat."""
    import sys

    action_name = str(action or "").strip().lower()
    path_list = [str(item) for item in (paths or []) if str(item).strip()]
    if not chat_id:
        return (
            "manage_visualization failed: missing chat_id. "
            "This tool is only available inside the UI Agent session."
        )

    ui_root = Path(__file__).resolve().parents[2] / "ui"
    if ui_root.is_dir() and str(ui_root) not in sys.path:
        sys.path.insert(0, str(ui_root))

    try:
        from viz_prefs import (  # type: ignore
        ...
        return f"manage_visualization failed: cannot import viz_prefs ({exc})"

    try:
        resolved = resolve_plot_paths(path_list) if path_list else []
        if action_name in {"show", "pin", "unhide", "add"}:
            if not resolved:
                return "manage_visualization failed: paths is required for show"
            prefs = show_figures(chat_id, resolved, pin=True)
            return (
                f"已在 Visualization 显示 {len(resolved)} 张图："
                + ", ".join(resolved)
                + f"。当前隐藏 {len(prefs.get('hidden') or [])} 张。"
            )
        if action_name in {"hide", "close", "remove"}:
            if not resolved:
                return "manage_visualization failed: paths is required for hide"
            prefs = hide_figures(chat_id, resolved)
            return (
                f"已从 Visualization 隐藏 {len(resolved)} 张图："
                + ", ".join(resolved)
                + f"。当前隐藏 {len(prefs.get('hidden') or [])} 张。"
            )
        if action_name in {"clear_hidden", "restore_all"}:
            prefs = clear_hidden_figures(chat_id)
            return f"已恢复全部隐藏图。当前 pinned {len(prefs.get('pinned') or [])} 张。"
        if action_name in {"list", "status"}:
            ...
            return (
                ...
                f"- pinned ({len(pinned)}): {', '.join(pinned) or '(none)'}"
            )
        return (
            "Unknown action. Use action=show|hide|list|clear_hidden "
            "with paths like results/plots/differential/volcano.png"
        )
    except Exception as exc:  # noqa: BLE001
        return f"manage_visualization failed: {exc}"
...
AGENT_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Control the UI right-rail Visualization board. "
                "Use when the user asks to show/hide/restore plots in the corner panel. "
                "action=show paths=[...] to display plots; action=hide to remove; "
                "action=list to inspect; action=clear_hidden to restore all closed plots. "
                "Prefer workspace-relative paths under results/plots/."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["show", "hide", "list", "clear_hidden"],
                    },
                    "paths": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": (
                            "Plot paths or filenames, e.g. "
                            "results/plots/differential/volcano.png or volcano.png"
                        ),
                    },
                },
                "required": ["action"],
