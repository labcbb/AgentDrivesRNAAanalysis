...
from sRNAgent.agent.orchestrator import run_agent_turn  # noqa: E402
    ...
    if event_type in {"code_execution_started", "code_execution_progress"}:
        with _runs_lock:
            ...
            for source, target in (
                ...
                ("progressOverallPct", "progressOverallPct"),
                ("progressFilePct", "progressFilePct"),
                ("progressRun", "progressRun"),
                ("progressFileIndex", "progressFileIndex"),
                ("progressFileTotal", "progressFileTotal"),
                ("progressBytes", "progressBytes"),
                ("progressBytesTotal", "progressBytesTotal"),
                ("progressLabel", "progressLabel"),
                ("progressIndeterminate", "progressIndeterminate"),
                ("progressCompletedFiles", "progressCompletedFiles"),
...
def kernel_figures(chat_id: str) -> Dict[str, Any]:
    from work_space import collect_workspace_figures
    from viz_prefs import apply_viz_prefs, load_viz_prefs

    figures: List[Dict[str, Any]] = []
    message = ""
    busy = False
    prefs = load_viz_prefs(chat_id) if chat_id else {"hidden": [], "pinned": []}

    # Prefer on-disk plots. Kernel inline history often repeats the same savefig
    # outputs and would duplicate Visualization cards.
    workspace_figures = collect_workspace_figures(limit=36)
    if workspace_figures:
        figures = workspace_figures
    else:
        execution = get_chat_execution(SRNAGENT_PROJECT, chat_id, create=False)
        if execution is None:
            message = "内核尚未启动"
        elif not execution.use_notebook or execution.notebook_executor is None:
            message = "Notebook 内核未启用"
        else:
            executor = execution.notebook_executor
            seen_data: set[str] = set()
            for item in executor.get_figures() or []:
                if not isinstance(item, dict):
                    continue
                data = str(item.get("data") or "")
                if not data:
                    continue
                key = data[:128] + f":{len(data)}"
                if key in seen_data:
                    continue
                seen_data.add(key)
                entry = dict(item)
                entry.setdefault("source", "kernel")
                figures.append(entry)

    execution = get_chat_execution(SRNAGENT_PROJECT, chat_id, create=False)
    if execution is not None and execution.use_notebook and execution.notebook_executor is not None:
        executor = execution.notebook_executor
        busy = bool(
            _chat_has_active_run(chat_id) or getattr(executor, "is_busy", lambda: False)()
        )
        if busy and not message:
            message = "Agent 正在运行，显示工作区图表"

    hidden_count = len(prefs.get("hidden") or [])
    visible = apply_viz_prefs(figures, prefs)
    if not visible:
        return {
            "ok": True,
            "ready": False,
            ...
            "prefs": prefs,
            "message": (
                f"已隐藏 {hidden_count} 张图，可点「恢复」或让 Agent 重新显示"
                if hidden_count
                else (message or "暂无图表输出")
            ),
        }
    return {
        "ok": True,
...
def update_viz_prefs(body: Dict[str, Any]) -> Dict[str, Any]:
    from viz_prefs import (
        clear_hidden_figures,
        hide_figures,
        load_viz_prefs,
        reorder_figures,
        resolve_plot_paths,
    ...
    raw_paths = body.get("paths") or body.get("path") or []
    if isinstance(raw_paths, str):
        raw_paths = [raw_paths]
    if not isinstance(raw_paths, list):
        raw_paths = []
    # Reorder uses the exact gallery sequence; resolve when possible but keep unknowns.
    if action in {"reorder", "sort", "order"}:
        paths = []
        for item in raw_paths:
            text = str(item or "").strip().replace("\\", "/")
            if not text:
                continue
            resolved = resolve_plot_paths([text])
            paths.append(resolved[0] if resolved else text)
        paths = list(dict.fromkeys(paths))
    else:
        paths = resolve_plot_paths([str(item) for item in raw_paths])

    try:
        if action in {"hide", "close"}:
            if not paths:
                return {"ok": False, "error": "paths 不能为空"}
            prefs = hide_figures(chat_id, paths)
        elif action in {"show", "unhide", "pin"}:
            if not paths:
                return {"ok": False, "error": "paths 不能为空"}
            prefs = show_figures(chat_id, paths, pin=action != "unhide")
        elif action in {"reorder", "sort", "order"}:
            if not paths:
                return {"ok": False, "error": "paths 不能为空"}
            prefs = reorder_figures(chat_id, paths)
        elif action in {"clear_hidden", "restore_all", "reset_hidden"}:
            prefs = clear_hidden_figures(chat_id)
    ...
    snapshot = kernel_figures(chat_id)
    return {
        "ok": True,
        ...
        "hiddenCount": snapshot.get("hiddenCount") or 0,
...
def agent_run_status(chat_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "codeIsDownloadTask": bool(code_state.get("isDownloadTask")),
        "codeProgressOverallPct": code_state.get("progressOverallPct"),
        "codeProgressFilePct": code_state.get("progressFilePct"),
        "codeProgressRun": code_state.get("progressRun") or "",
        "codeProgressFileIndex": code_state.get("progressFileIndex"),
        "codeProgressFileTotal": code_state.get("progressFileTotal"),
        "codeProgressBytes": code_state.get("progressBytes"),
        "codeProgressBytesTotal": code_state.get("progressBytesTotal"),
        "codeProgressLabel": code_state.get("progressLabel") or "",
        "codeProgressIndeterminate": bool(code_state.get("progressIndeterminate")),
        "codeProgressCompletedFiles": code_state.get("progressCompletedFiles"),
        "codeProgressStage": code_state.get("progressStage") or "",
...
def run_agent_chat(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "meta": {
            ...
            "backend": "sRNAgent",
            "orchestrator": turn.get("backend")
            or ("langgraph" if langgraph_orchestrator_enabled() else "legacy"),
            "tool_loop": "langgraph" if lc_tool_loop_enabled() else "legacy",
            "plan_graph": "langgraph" if lc_plan_graph_enabled() else "legacy",
            "route_intent": turn.get("route_intent") or route_decision.intent.value,
            ...
            on_progress(
                {
                    ...
                    "meta": {
                        ...
                        "tool_loop": "langgraph" if lc_tool_loop_enabled() else "legacy",
                        "plan_graph": "langgraph" if lc_plan_graph_enabled() else "legacy",
                        "route_intent": turn.get("route_intent")
                        or route_decision.intent.value,
                        "execution": agent.execution.to_dict(),
                    },
                }
            )
