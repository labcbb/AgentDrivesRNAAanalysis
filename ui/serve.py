...
from work_space import configure_work_space, resolve_work_space_path, WorkSpacePathError
from jupyterlab_manager import (
    get_jupyterlab_status,
    start_jupyterlab,
    stop_jupyterlab,
)
...
def forward_viz_prefs(body: dict[str, Any]) -> dict[str, Any]:
    try:
        from agent_bridge import update_viz_prefs

        return update_viz_prefs(body)
    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "error": f"viz prefs error: {exc}"}
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/work_space/files":
            ...
            return
        if path == "/api/work_space/raw":
            rel = str((query.get("path") or [""])[0]).strip()
            if not rel or rel in {".", "/"}:
                self.send_error(400, "path 不能为空")
                return
            try:
                file_path = resolve_work_space_path(rel)
            except WorkSpacePathError:
                self.send_error(400, "路径必须在 work_space 内")
                return
            except Exception as exc:  # noqa: BLE001
            ...
            try:
                ...
                self.send_error(500, f"读取失败: {exc}")
                return
            self.send_response(200)
        ...
        if path == "/api/supervisor/report":
            ...
            return
        if path == "/api/jupyterlab/status":
            self._write_json(get_jupyterlab_status())
            return
        super().do_GET()
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/api/kernel/figures/prefs":
            self._write_json(forward_viz_prefs(body))
            return
        ...
        if path == "/api/jupyterlab/start":
            force = bool(body.get("force") or body.get("restart"))
            self._write_json(start_jupyterlab(force_restart=force))
            return

        if path == "/api/jupyterlab/stop":
            self._write_json(stop_jupyterlab())
            return
...
def main() -> None:
    ...
    print("sRNAgent    → POST /api/agent/chat  /api/agent/chat/stream  …")
    print("JupyterLab  → GET /api/jupyterlab/status  POST /api/jupyterlab/start|stop  (Overview 独立内核, 默认端口 8766)")
